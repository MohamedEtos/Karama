<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="usercode" class="block mt-1 w-full" type="text" name="usercode" :value="old('usercode')" required autofocus autocomplete="usercode" />
        ...
        <div>
            <x-input-label for="subtype" :value="__('subtype')" />
            <select  id="subtype" name="subtype">
                <section value="user">:value="old('subtype')"</section>
                <option value="user">مستخدم</option>
                <option value="merchant">تاجر</option>
                <option value="admin">ادمن</option>
            </select>
            <x-input-error :messages="$errors->get('subtype')" class="mt-2" />
        </div>
